...
| V-6 | PARTIAL | S3 (Ceph RGW) versioning + object lock + lifecycle verified on store A; Swift on a separate Brno endpoint verified as store B; `COLD` storage class rejected on upload → cold tier stays a profile value |
...
**Still open:** V-1, V-2, V-3 (Phase 4 / documentation tasks); cold-tier storage class name on CESNET S3 (V-6); whether Rancher can mint short-lived tokens for a CI principal (V-8); D-1, D-3, D-4, D-5, D-8 … D-12 (deferred by design); D-2 engine choice (ADR before Phase 2 schema). No `CLAUDE.md` exists yet (Phase 0 deliverable); the repository was `git init`-ed this session with docs only.
