...
and deadline. The manual drill after that (revision 28, 03:01–03:46 UTC) passed both phases
on the live data: 718.6 s against the restored database, 1 923.2 s for the Bronze-only
rebuild, every version of every target reproduced (`docs/07` §5.4). Left for the author: a real receiver by values, the night-time
