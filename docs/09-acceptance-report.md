...
4. **PR #5 (`ote_imbalance_settlement_final`, version 2, run 4) — reviewed 2026-09-24, merge is
   the author's** (`gh pr merge 5 --admin --squash --delete-branch`; the push deploys it). The
   review is the run 4 section above. Once merged, the dataset has all three settlement versions
   as target-only adapters and F-4 is closed in full.
