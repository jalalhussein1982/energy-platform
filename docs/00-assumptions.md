...
| A-9 | Data centre is **in the Czech Republic** | CESNET sites (Prague, Brno) | Residency is a declared attribute (`residency: CZ`) asserted at deploy time and documented; never encoded in platform code *(2026-09-19, ADR-028: the demo environment declares `residency: DE`, shown as the attribute working; production unchanged)* | Y / Y / C only if region-pinned | None | — |
...
*(2026-09-19: the proposals below were accepted as ADR-003 rev. and ADR-016 and later amended — ADR-003 rev. by ADR-024 (fencing, attempts, reconcile, backfill), ADR-016 §2 by ADR-025 (hook chain instead of `helm test`) and §4 by ADR-023 (`--derivation`). The text below is the original proposal, kept as history.)*
...
| V-11 | *(ADR-026, 2026-09-19)* Does the tenant CNI **enforce** egress `NetworkPolicy`? (V-4 showed only that policies can be created) | Apply a default-deny egress policy to a scratch pod in `$NS`; `kubectl exec … -- curl -m 5 https://www.ote-cr.cz/en` must fail; remove the policy, must succeed; `kubectl get nodes -o wide` / CNI pods in `kube-system` for the record | ADR-026 layer 1 status on the `tenant` profile |
| V-12 | *(ADR-028, 2026-09-19)* Hetzner Object Storage as Bronze store A: versioning, **Object Lock**, storage classes, lifecycle | `aws --endpoint-url $S3_A s3api put-bucket-versioning --bucket $B --versioning-configuration Status=Enabled` · `put-object-lock-configuration` then `delete-object --version-id` on a locked object must fail · storage-class loop and `put-bucket-lifecycle-configuration` with a bogus class, exactly as V-6 | ADR-028 §3: Object Lock on A, or the ADR-021 compensating control |
| V-13 | *(ADR-028, 2026-09-19)* OCI Object Storage (Frankfurt) via the S3-compatible endpoint as Bronze store B: versioning, **retention rule** enforced, cross-provider copy | `aws --endpoint-url $S3_B s3api get-bucket-versioning` · create a retention rule in the OCI console/Terraform, `delete-object` inside the window must fail · `rclone sync` A → B on a fixture set, then `rclone check --download` for checksum match; note wall-clock and egress cost | ADR-002/ADR-021 §4 independent copy on the demo; restore-drill read from B |
| V-14 | *(ADR-028, 2026-09-19)* k3s structured authentication trusting the GitHub Actions OIDC issuer | k3s `--kube-apiserver-arg authentication-config=…` with the `token.actions.githubusercontent.com` issuer; a workflow job exchanges its ID token, then `kubectl auth can-i --list -n $NS` shows namespace verbs and `kubectl auth can-i create namespaces` → no | ADR-015 federation clause vs two-token fallback on the demo cluster |
...
- [x] *(2026-09-19, ADR-028 ACCEPTED)* `02` ADR-001: reference environment hosts no release; demo-environment paragraph added; `tenant` bullet points at the demo cluster. `02` §4.2 D-1 (two Terraform roots), D-3 (demo store A/B). `02` §4.3 and this file §4 gain V-11 … V-14. §2.1 `own-cluster` row names both roots; A-9 gains the `residency: DE` note. `03` Phase 5 and Phase 8 reworded (done at proposal time).
