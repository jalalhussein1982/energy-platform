...
2026-09-19  V-6   PARTIAL    store A = Ceph RGW (squid) at s3.cl4.du.cesnet.cz (CESNET Data Care): versioning Enabled, object lock COMPLIANCE/GOVERNANCE accepted and enforced (delete → AccessDenied "forbidden by object lock"), lifecycle rules accepted; store B = Swift on a separate RGW at object-store.brno.openstack.cloud.e-infra.cz (MetaCentrum Cloud Brno1), account reachable, 5 GB quota; but put-object with StorageClass COLD rejected on store A   A-3 CONFIRMED for the S3-API contract and for the second, independently operated endpoint (different site, different operator team, different credential system: S3 keys vs application credential). Only the cold tier is unresolved → ADR-002 keeps "lifecycle to cold tier" as a profile value (`bronze.coldStorageClass`, empty = no transition), never a hard requirement; Bronze replication stays an `rclone` job (S3 → Swift). Would resolve: CESNET's documented storage-class name for the cold tier (or `radosgw-admin zonegroup get` from the operator). rclone not run (absent locally; not run over SSH to avoid passing keys through a shared front-end); `aws s3api` covers the same evidence.
