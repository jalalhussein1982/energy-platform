...
Alerting is evaluated, not only defined. A Prometheus in the namespace loads the same twelve rules the chart has shipped as a ConfigMap since the freshness design, scrapes the exporter and a namespaced kube-state-metrics, and hands firing and resolved alerts to an Alertmanager that delivers them to a receiver on the platform image; its log is the delivery record. Delivery and notification are kept apart: the log receives every alert, and a person receives what pages. On the demo that person's channel is a mailbox reached through an authenticated submission service; the chart takes the channel's credential from a Secret the operator creates, refusing at render a credential typed into values or a route to a receiver that does not exist. The delivery drill creates a restore-drill Job that fails on purpose and waits for the `firing` and then the `resolved` delivery; it runs on the laptop profile as part of the gate, passed on the demo on 25 September into the log (firing delivered two minutes after the failure, resolved one minute after the deletion), and on 26 September into the log and the mailbox in the same second, under three minutes from the failure to the inbox (`docs/07-operations.md` §7.3). Its first live evaluation also corrected the two Job-failure rules, which had paged for day-old failed Jobs kept as records after a later success.
...
| alert delivery drill | pass on kind and on the demo into the log, 2026-09-25; pass on the demo into the log and the operator's mailbox, 2026-09-26 | `docs/07-operations.md` §4.4, §7.2, §7.3 |
...
| live demo | Helm revision 29, 33 CronJobs, capturing since 2026-09-23; the alert channel wired to a mailbox since 2026-09-26 | `docs/07-operations.md` §4, §7.3 |
...
- **Two freshness alerts are routed to the log, not to a person.** A person is paged on the demo since 2026-09-26: the channel is a mailbox behind an authenticated submission service, its credential in a Secret the maintainer created, and the delivery drill ended in that inbox (ADR-040 amendment 1, `docs/07` §7.3). What remains is a calibration question the first live night raised: the freshness rule paged at 01:45 for the day-ahead and the daily settlement targets, whose publication expectations the code documents as uncalibrated. The interim is routing, not a wider window: the demo's values keep those two alerts in the log and page everything else; the fix is a publication expectation in the manifest, planned and not built.
