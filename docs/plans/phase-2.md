...
## Appendix — Task 2.10: S3 backend (2026-09-20, ADR-032 ACCEPTED Option B)

Planned after ADR-032 was accepted; the last Phase 2 deliverable. Phase 3 does not start here.

| | |
|---|---|
| Goal | `Bronze` runs on an S3-compatible object store (MinIO ×2 in Phase 5's local profile, Hetzner/OCI in the demo) behind the Phase 2 `BlobStore` / `CaptureLog` protocols, with no new dependency and every outbound HTTP byte still inside `energy_platform/fetch/`. |
| Spec | ADR-032 (Option B), ADR-002 (key layout), ADR-021 (hot → cold read, `tier`), ADR-026 §2 (egress is validated before contact), ADR-027 §3 (single egress package), `00` §5 V-6 (object-lock headers must be sent). |
| Do not | Add a dependency. Import `httpx` outside `fetch/`. Open a socket in a test. Read credentials from a manifest. Delete an object except behind an explicit flag reserved for the tiering job. Touch the `Bronze` facade, the fixture format or the capture-log layout. |

### Decisions taken by this appendix

| # | Decision | Why |
|---|---|---|
| P2-D15 | The object-store client is `energy_platform/fetch/objectstore.py`: SigV4 (`AWS4-HMAC-SHA256`, service `s3`, unsigned payload never — `x-amz-content-sha256` is always the payload hash), **path-style** addressing (`/<bucket>/<key>`), verbs `PUT`, `GET`, `HEAD`, `LIST` (ListObjectsV2 with `continuation-token` pagination) and `DELETE`. `DELETE` raises unless the store was built with `allow_delete=True`; the tiering job (Phase 5) is the only caller that sets it. | ADR-032 Option B names exactly these verbs; a Bronze writer never deletes (ADR-002 immutability, ADR-021 §"devil's advocate"). Path-style works on MinIO, Ceph RGW, Hetzner and OCI without DNS games. |
| P2-D16 | Endpoint policy is its own check, not `fetch.policy.check_url`: the endpoint host must be in a **configured allowlist** (exact, lowercase; an in-cluster name or a loopback literal for the local profile is legitimate there), `https` unless `allow_insecure`, no userinfo/query/fragment. The platform host registry (sources) and the private-range rejection (ADR-026 layer 2) do not apply: the object store is by design a private destination that `NetworkPolicy` layer 1 allows explicitly. Retries reuse `fetch.client.RetryPolicy` (connection errors and 5xx only). | ADR-026 §1 already carves the object store out of layer 1's public-only rule; layer 2 exists to keep *sources* honest. Two allowlists (sources, stores) are clearer than one with exceptions. |
| P2-D17 | Credentials come from the environment through the existing `secretRef` resolver: a store named `<name>` reads `<NAME>_ACCESS_KEY_ID`, `<NAME>_SECRET_ACCESS_KEY` and optionally `<NAME>_SESSION_TOKEN` (`fetch.secrets.env_name` on `SecretRef(name, key)`), which is what the chart's `Secret` will map to. Endpoint, bucket, region and allowlist are configuration values, never part of a manifest. The secret key is never logged; the `Authorization` header is redacted from errors. | Prompt: "secretRef-style names, never from a manifest"; ADR-017. |
| P2-D18 | `PUT` sends `Content-MD5` always and `x-amz-object-lock-mode` / `x-amz-object-lock-retain-until-date` when the store is configured with a retention (`mode ∈ {COMPLIANCE, GOVERNANCE}`, `retain_days`); S3 requires the MD5 with object-lock headers. `S3BlobStore.put` does `HEAD` first and skips the `PUT` when the content address already exists. | V-6: the client must send object-lock headers; a second `PUT` of a locked, versioned object is a wasted version at best and a `403` at worst. Idempotency is by content address, as in the memory/file backends. |
| P2-D19 | `S3CaptureLog` derives keys from names: `get(capture_id)` parses `<target>:<stamp>:<attempt>` back into `entry_key` (one `GET`, no listing); `entries_for` lists the day prefix `captures/<t>/<Y>/<M>/<D>/<stamp>_`; `list`/`latest` list `captures/<t>/`, filter by the stamp **in the key** and fetch only the matching entries. The `tier` field round-trips untouched (the tiering job rewrites it later). | Listing is the only S3 primitive for "attempts from the prefix" (ADR-024 §1); keys already carry `scheduled_for` and `attempt`, so a window filter needs no `GET`. |
| P2-D20 | Tests use a fake S3 built on `fetch.offline.mock_transport`: an in-memory bucket that checks path-style URLs, the `x-amz-content-sha256` header against the body and the `Authorization` shape, records every request, and answers with MinIO-shaped XML (ListObjectsV2 pages, `NoSuchKey`). SigV4 is asserted against the AWS documentation vector (`GET /test.txt`, `20130524T000000Z`, `AKIAIOSFODNN7EXAMPLE`: canonical-request hash `7344ae5b…6972`, signature `f0e8bdb8…db41`). | ADR-032 "offline tests against a recorded MinIO exchange"; a known-answer test is the only way to prove a signer without a network. |

### Steps

**Files:** `energy_platform/fetch/objectstore.py`, `energy_platform/fetch/__init__.py` (exports), `energy_platform/bronze/s3.py`, `energy_platform/bronze/__init__.py` (exports, docstring), `tests/fetch/fake_s3.py`, `tests/fetch/test_objectstore.py`, `tests/bronze/test_s3.py`, `docs/adr/ADR-027-target-capability-boundary.md` §3 (one sentence), `docs/adr/ADR-032-object-store-client.md` (status line: implemented), `docs/04-contracts.md` §6 (where the tests are), `docs/03-roadmap.md` Phase 2, `docs/progress.md`.

- [x] 2.10.1 Plan (this appendix). Commit `docs(plan): Task 2.10 — S3 backend appendix (ADR-032 Option B)`.
- [x] 2.10.2 `fetch/objectstore.py`: `ObjectStoreConfig` (endpoint, bucket, region, `allowed_hosts`, `allow_insecure`, `allow_delete`, `retention`), `Credentials.from_env(name)`, `sign_v4(...)` as a pure function, `ObjectStore` with `put/get/head/list/delete`, `ObjectStoreError(code)`; `check_endpoint`. Tests: KAT (canonical request, string to sign, signature), header set per verb, path-style URL, content hash, object-lock headers with the retain-until date from the injected clock, `Content-MD5`, allowlist/scheme refusal before any request, 404 → `None` for `get`/`head`, `list` follows `NextContinuationToken` across three pages, `delete` refused without the flag, retry on 5xx and connection error, the unit-test socket block refuses a real transport, secret never in an error message. Commit `feat(fetch): minimal SigV4 object-store client over httpx — PUT/GET/HEAD/LIST/DELETE, path-style, object-lock headers, endpoint allowlist (ADR-032)`.
- [x] 2.10.3 `bronze/s3.py`: `S3BlobStore`, `S3CaptureLog`. Tests: key layout on the wire (`bronze/blobs/<sha[:2]>/<sha>`, `captures/<t>/<Y>/<M>/<D>/<stamp>_<n>.json`), `put` idempotent (HEAD then PUT, second `put` sends no `PUT`), `get` missing → `None`, `Bronze` hot → cold with two `S3BlobStore` instances reporting `tier="cold"`, entry round trip including `tier="cold"`, `entries_for`/`list`/`latest` ordering and window across a paginated listing, `capture` end-to-end through `Bronze` on the fake (blob then entry order observed in the request log), fixture `ingest`. Commit `feat(bronze): S3 blob store and capture log over fetch.objectstore, hot→cold with a second store (ADR-002, ADR-021, ADR-032)`.
- [x] 2.10.4 Docs: ADR-027 §3 one sentence naming `fetch/objectstore.py`; ADR-032 status "implemented 2026-09-20"; `bronze/__init__` docstring; `04` §6 row; roadmap Phase 2 box un-struck and ticked; `progress.md` entry. `make check`, `make deps-allowlist`, `make secret-scan`. Commit `docs(roadmap): Phase 2 closed — S3 Bronze backend shipped (ADR-032)`.

### Stop conditions

- Signing needs a header the fake cannot verify offline → record it, keep the KAT as the proof.
- Any test needs a MinIO process → it belongs to Phase 5 `local-up`, not here.
