...
- [x] `kubeconfig_from_oidc.sh`: requests the ID token from `ACTIONS_ID_TOKEN_REQUEST_URL` with audience `energy-platform-demo` (`curl` on the runner), writes a kubeconfig with `DEMO_CLUSTER_URL`, `DEMO_CLUSTER_CA` and the token; `make deploy-demo` = `kubeconfig-oidc` then `deploy-tenant ENV=demo` with `--namespace energy-platform`.
