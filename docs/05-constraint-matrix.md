...
| C-51 | **`open_pr` on a mixed, incomplete or red target** | mcp/`harness.pr`: bundle only when surface, admission and goldens are green and the session touched one target; the report and the body name the checks the bundle does not run (target pytest, ruff, mypy → CI), so a green bundle is not read as a full verdict (review 2 AE-03) | `test` | `test_mcp.py::test_open_pr_refuses_when_gates_are_red` |
...
| C-58 | **Triage model answer outside the closed operation schema** (a unit, sign, host, URL, dataset or licence change; extra keys; a forbidden operation smuggled next to an allowed one; not JSON) — the model may be obeying an injection in the captured document (ADR-008 items 1, 2) | `energy_platform.triage.pipeline.parse_answer` / `constrain`: the whole answer is refused | `test` | `tests/triage/test_injection.py::test_hostile_xml_cannot_change_unit_sign_host_or_url` (10 obeying answers), `tests/triage/test_injection.py::test_hostile_html_cannot_widen_the_patch` |
...
| C-63 | **A target's `secretRef` naming a platform or another target's credential** (`{name: BRONZE, key: secret-access-key}` resolves `BRONZE_SECRET_ACCESS_KEY` and sends the store-A key to a registered host) (ADR-017, threat model §4) | manifest: `auth.secretRef.name` must be `target-<target_id>`, key alphanumeric; fetch: `ScopedSecretResolver` inside `fetch_for_manifest` refuses any other name, validated or not | `harness-check`; runtime | `tests/contracts/test_manifest_negative.py::test_secret_ref_must_be_the_targets_own`, `tests/contracts/test_manifest_negative.py::test_secret_ref_key_must_be_alphanumeric`, `tests/fetch/test_plan.py::test_scoped_resolver_refuses_a_platform_or_another_targets_secret`, `tests/fetch/test_plan.py::test_unvalidated_manifest_naming_a_platform_secret_sends_nothing` |
| C-64 | **Unbounded response body** (a huge or never-ending answer, a compressed bomb, a `Content-Length` that lies) exhausting the capture pod's memory (threat model §2; ADR-026) | fetch: every response is streamed; a declared `Content-Length` above `max_body_bytes` (default 64 MiB) is refused before reading, a streamed body is counted after content decoding and aborted past the cap; `response_too_large` is never retried; error bodies are read to 2 KiB | `test` | `tests/fetch/test_client.py::test_declared_length_above_the_cap_is_refused_before_reading`, `tests/fetch/test_client.py::test_streamed_body_without_a_length_is_counted_and_stopped`, `tests/fetch/test_client.py::test_body_exactly_at_the_cap_is_accepted`, `tests/fetch/test_client.py::test_error_body_is_read_to_2_kib_only` |
| C-65 | **Payload pod working before its NetworkPolicy is in force** (a CNI that applies a new pod's policy asynchronously lets its first packets out: on the demo's kube-router a fresh pod reached the metadata service, which serves the k3s join token) (ADR-026 amendment 2) | chart `egress.policyGate`: `energyctl wait-egress-policy` as the first init container of every platform-image pod; opens after the metadata canary is refused twice in a row, fails closed after `timeoutSeconds` | `test`; runtime | `tests/fetch/test_policy_gate.py::test_gate_waits_through_the_unfiltered_start_then_opens`, `tests/fetch/test_policy_gate.py::test_a_cluster_that_never_blocks_the_canary_fails_closed`, `tests/harness/test_chart.py::test_policy_gate_runs_first_in_every_payload_pod_behind_its_flag` |
| C-66 | **A capture of one delivery day holding another day's payload** (a discovery page that lists the newest file; conditional validators, a 304 or `content_changed` taken from the target's newest run) — the mapping dates the rows with the run's day (ADR-033 amendment 2) | fetch: a discovered link must name the day's template file; runtime: the baseline is the same day's newest HTTP 200 entry; Bronze: a 304 for another URL than the baseline is refused; alert `EnergyPlatformReconciliationMismatch` | `test`; runtime | `tests/fetch/test_plan.py::test_t2_discovered_link_for_another_day_falls_back_to_the_days_template`, `tests/runtime/test_recapture.py::test_a_correction_compares_with_its_own_day_not_the_newest_run`, `tests/bronze/test_bronze.py::test_304_for_another_resource_than_the_baseline_is_refused` |
| C-67 | **Sandbox image drifting from ADR-007 mode 2** (a shell left in the image, an operator tool or credential installed, root user, an entrypoint other than the MCP server, `--allow-network` baked in, a base image by tag) | static checks over `deployment/sandbox/Dockerfile`'s final stage | `test` | `tests/harness/test_sandbox_image.py::test_the_final_image_removes_every_shell_and_the_installer`, `tests/harness/test_sandbox_image.py::test_no_operator_tool_or_credential_is_installed`, `tests/harness/test_sandbox_image.py::test_runs_unprivileged_with_the_mcp_server_as_the_only_entrypoint`, `tests/harness/test_sandbox_image.py::test_every_base_image_is_pinned_by_digest` |
| C-68 | **External Postgres mode with no destination** (`postgres.mode=external` and `egress.postgres.cidrs` empty: an empty `to:` list is every destination in the NetworkPolicy API, so TCP 5432 would open to the world for every verb pod — review 2 DEP-04) | chart | `helm-lint` | `tests/harness/test_chart.py::test_external_postgres_mode_refuses_an_empty_destination_list` |
| C-69 | **Known-wrong output coming back through a rebuild** (Silver rows deleted by hand, a runbook note "do not replay": the restore drill re-processes every capture and reports the deleted rows as pending work — review 2 DC-07) | runtime (ADR-038: invalidation objects in Bronze, mirrored by reconcile; the views, process, replay and the drill honour them) | `test` | `tests/runtime/test_review2.py::test_dc07_an_invalidated_capture_is_not_current_not_replayed_and_not_restored`, `tests/store/test_store.py::test_review2_dc07_an_invalidated_capture_leaves_the_current_view_without_deletion` |
| C-70 | **A viewer that publishes or can write** (a dashboard on a public address shows values the sources allow for internal use only, `06` §1.4/§4.5; a dashboard holding the application's DSN can write Silver) | chart (ADR-039: ClusterIP + NetworkPolicy only, no Ingress; a read-only login role in `energy_reader`, migration 0007, granted by the migrate hook; provisioned, stateless) | `helm-lint`, `db-test` | `tests/harness/test_chart.py::test_grafana_is_off_by_default_and_private_behind_its_flag`, `tests/store/test_migrations.py::test_reader_role_can_select_and_cannot_write` |
| C-71 | **A local object store that only looks like one** (a replacement server that accepts the `x-amz-object-lock-*` headers and stores nothing, or a pinned image a publisher has withdrawn: the store-A control of ADR-036 §1 becomes a lie, or the profile cannot start) | ADR-036 amendment 4: chosen by an empirical probe (lock, versioning, refused delete, paging, `If-None-Match`, rclone `--immutable`) before any chart change; the bucket is created by the platform's own client (`energyctl bucket-init`), no CLI image | `helm-lint`, `check` | `tests/harness/test_chart.py::test_local_object_stores_are_rustfs_with_the_bucket_init_hook_on_the_platform_image`, `tests/fetch/test_objectstore.py::test_bucket_init_creates_a_locked_versioned_bucket_once`; the probe transcript in `docs/07` §8.4 |
| C-72 | **A scheduled capture keyed by the pod's start time** (`capture --live` without an instant used `now()`, the gap detector expects the cron instant: every good capture a phantom `missing_capture`, the backfill fetching the same day again — review 3 R2) | runtime (ADR-031 amendment 1: the CronJob controller's tick from the Job name by the downward API, else the newest firing instant); chart passes `ENERGY_PLATFORM_JOB_NAME` | `test`, `helm-lint` | `tests/runtime/test_review3.py::test_r2_a_capture_started_25_seconds_late_serves_its_tick_and_nothing_is_refetched`, `tests/runtime/test_review3.py::test_r2_a_genuinely_missed_tick_is_still_detected`, `tests/cli/test_cli.py::test_r2_live_capture_without_an_instant_is_keyed_by_the_schedule`, `tests/harness/test_chart.py::test_r2_the_capture_container_learns_its_job_name_by_the_downward_api` |
| C-73 | **A code fix that replays as a no-op** (the derivation identity named a package version nobody bumps, so a corrected parser or mapping upserted under the old key — review 3 R4) | runtime (ADR-023 amendment 3: `implementation_version()` = package version + source digest in every derivation identity and generic parser ref) | `test`, `db-test` | `tests/runtime/test_review3.py::test_r4_the_digest_follows_the_package_bytes_and_ignores_bytecode`, `tests/runtime/test_review3.py::test_r4_a_corrected_implementation_replays_as_a_new_derivation_and_the_same_one_is_a_noop`, `tests/store/test_store.py::test_review3_r4_the_stored_derivation_names_the_implementation` |
| C-74 | **A restore drill whose verdict depends on order or on a warm scratch store** (targets sharing a dataset compared before their siblings were rebuilt; every stored version demanded with its derivation id, so a value-preserving revision failed — review 3 R5, R6) | runtime (ADR-036 amendment 5: the cohort rebuilt first, lineage-scoped value fingerprints, `missing` / `diverging` / `historical` named) | `test`, `db-test` | `tests/runtime/test_review3.py::test_r5_targets_sharing_a_dataset_pass_a_fresh_rebuild_in_either_order`, `tests/runtime/test_review3.py::test_r6_a_value_preserving_mapping_revision_is_history_not_loss`, `tests/runtime/test_review3.py::test_r6_a_value_changing_implementation_diverges_until_replayed`, `tests/store/test_review3_drill.py` |
| C-75 | **Alert rules nobody evaluates** (a rules ConfigMap "for any scraper", five rules over `kube_*` metrics nothing exported, no Alertmanager, no receiver — review 3 R3) | chart (ADR-040: `alerting.enabled` renders Prometheus over the same ConfigMap, a namespaced kube-state-metrics with a Role only, Alertmanager to the platform receiver; the API-server address required; no CRD, no ClusterRole) | `helm-lint`, `test`; the delivery drill on kind (`make alert-drill`, `07` §7.2) | `tests/harness/test_chart.py::test_alerting_renders_behind_its_flag_without_cluster_rights`, `tests/fetch/test_alert_sink.py` |
...
| `test` | pytest with sockets disabled; `tests/harness/test_goldens.py` auto-discovers every `targets/*/tests/golden/*.yaml`; `conftest.py` refuses an incomplete target at session start | C-07, C-09, C-14…C-17, C-31…C-37, C-46…C-53, C-58…C-67 |
| `harness-check` | `validate-targets` (`energyctl validate --all`), `migration-check`, `workload-check` | C-04, C-19…C-29, C-42…C-45, C-54, C-62, C-63 |
